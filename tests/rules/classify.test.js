...
describe("classify", () => {
  ...
  it("應該優先套用黑名單規則", () => {
    ...
    expect(result.danger.length).toBe(2);
    result.danger.forEach((item) => {
      expect(item.matchType).toBe("blacklist");
      expect(item.item.source).toBe("blacklist");
      expect(item.item.vegetarian).toBe(false);
      expect(item.item.vegan).toBe(false);
    });
  });
  ...
  it("應該優先套用白名單規則", () => {
    ...
    expect(result.safe.length).toBeGreaterThanOrEqual(2);
    result.safe.forEach((item) => {
      if (["水", "糖"].includes(item.input)) {
        expect(item.matchType).toBe("whitelist");
        expect(item.item.source).toBe("whitelist");
        expect(item.item.vegetarian).toBe(true);
        expect(item.item.vegan).toBe(true);
      }
    });
  });
});
